...
COMMENT ON COLUMN "customer_external_accounts"."provider" IS 'Канал идентификации клиента: telegram/web/mini_app/max/other';
...
COMMENT ON COLUMN "orders"."source_channel" IS 'Канал создания заказа: telegram/web/mini_app/max/other';
...
COMMENT ON COLUMN "orders"."status" IS 'Статус заказа: created/awaiting_payment/confirmed/needs_clarification/paid/in_progress/shipped/cancelled';
...
COMMENT ON COLUMN "payments"."payment_id" IS 'Внутренний идентификатор попытки оплаты в Kircraft';
